# AN0384 — golden replay datasets

`Disk Wiping and Partition Destruction Utilities (AN0384)`
...
| `benign_1.jsonl` | 0 | near-miss value for 'cmd_line contains clean' — every other condition holds, so the stage gate rejects the events |
| `benign_2.jsonl` | 0 | near-miss value for 'image_path regex .+\Q\diskpart.exe\E$' — every other condition holds, so the stage gate rejects the events |
