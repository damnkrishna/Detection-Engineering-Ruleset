# 2376b39c-0981-4bed-bc5c-dea5a81adabc — golden replay datasets

`Unauthorized Raw Disk Partition Access (AN0827-B)`
...
| `benign_1.jsonl` | 0 | near-miss value for 'image_path not_regex .*\Q\acronis.exe\E$' — every other condition holds, so the stage gate rejects the events |
| `benign_2.jsonl` | 0 | near-miss value for 'device contains \PhysicalDrive' — every other condition holds, so the stage gate rejects the events |
