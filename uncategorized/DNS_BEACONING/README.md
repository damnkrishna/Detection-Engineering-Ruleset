# DNS_BEACONING — golden replay datasets

`Repeated suspicious DNS queries (DGA / DNS tunnelling) from same host`
...
| `benign_2.jsonl` | 0 | the final event falls outside the 1200s window, so the anchored window restarts and the count never accumulates |
