# 2601ce9e-fa47-4bf2-85b1-cbcff931ec0a — golden replay datasets

`Unofficial DHCP Server Port Binding (AN1290-B)`
...
| `benign_1.jsonl` | 0 | near-miss value for 'image_path not_regex .*\Q\vboxsvc.exe\E$' — every other condition holds, so the stage gate rejects the events |
| `benign_2.jsonl` | 0 | near-miss value for 'src_port in_list [67]' — every other condition holds, so the stage gate rejects the events |
